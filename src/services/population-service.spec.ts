import { WorldBankApiV2CountryResponse } from 'services';
...
describe('population service', () => {
  it('should succeed if the URL is provided and valid', () => {
    expect(new PopulationServiceImpl('https://foo')).toBeInstanceOf(
      PopulationServiceImpl
    );
    expect(new PopulationServiceImpl('https://foo/')).toBeInstanceOf(
      PopulationServiceImpl
    );
    expect(new PopulationServiceImpl('http://foo')).toBeInstanceOf(
      PopulationServiceImpl
    );
    expect(new PopulationServiceImpl('http://foo/')).toBeInstanceOf(
      PopulationServiceImpl
  ...
  test('should not accept null input', () => {
    expect(
      () => new PopulationServiceImpl((null as unknown) as string)
    ).toThrow();
    expect(
      () => new PopulationServiceImpl((undefined as unknown) as string)
    ).toThrow();
  });
  ...
  describe('getAllCountries', () => {
    const dummyValidData: WorldBankApiV2CountryResponse = [
    ...
    it('should succeed and return countries when the request succeeds and a valid response is received', async () => {
      fetchMock.mockResponse(JSON.stringify(dummyValidData), { status: 200 });
      ...
      await sut
        ...
        .then((countries) => {
          ...
          expect(countries[0].capitalCity).toBe(
            dummyValidCountries[0].capitalCity
          );
          expect(countries[0].name).toBe(dummyValidCountries[0].name);
          expect(countries[1].capitalCity).toBe(
            dummyValidCountries[1].capitalCity
          );
    ...
    it('should fail and throw if the response does not match the expected format', async () => {
      fetchMock.mockResponse(JSON.stringify({ foo: 'bar' }), { status: 200 });
    ...
    it('should check the response status and fail if outside of the allowed range (2xx)', async () => {
      fetchMock.mockResponse(JSON.stringify(dummyValidData), { status: 199 });
      await expect(sut.getAllCountries()).rejects.toBeTruthy();

      fetchMock.mockResponse(JSON.stringify(dummyValidData), { status: 300 });
      await expect(sut.getAllCountries()).rejects.toBeTruthy();
    });
